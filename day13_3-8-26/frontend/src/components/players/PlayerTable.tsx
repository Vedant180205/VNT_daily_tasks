  ...
  return (
    <Card className="w-full">
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-border bg-gray-50/50">
              <th className="px-6 py-4 text-[12px] font-bold text-muted uppercase tracking-wider">Player</th>
              <th className="px-6 py-4 text-[12px] font-bold text-muted uppercase tracking-wider hidden sm:table-cell">Team</th>
              <th className="px-6 py-4 text-[12px] font-bold text-muted uppercase tracking-wider hidden md:table-cell">Contact</th>
              <th className="px-6 py-4 text-[12px] font-bold text-muted uppercase tracking-wider">Status</th>
              <th className="px-6 py-4 text-[12px] font-bold text-muted uppercase tracking-wider hidden lg:table-cell">Joined</th>
          ...
          >
            {players.map((player) => (
              ...
              >
                ...
                <td className="px-6 py-3">
                  <StatusBadge status={player.is_deleted ? 'inactive' : 'active'}>
                    {player.is_deleted ? 'Inactive' : 'Active'}
                  </StatusBadge>
                </td>
                ...
                <td className="px-6 py-3 text-right">
                  <div className="flex items-center justify-end gap-2">
                    <button 
                      ...
                      className="px-3 py-1.5 rounded-md text-[13px] font-semibold text-indigo-600 bg-indigo-50 hover:bg-indigo-100 transition-colors border border-indigo-100"
                    >
                      View
                    </button>
                    <button 
      ...
      <PlayerDetailsDialog 
        ...
        open={viewingPlayerId !== null}
        onOpenChange={(open) => !open && setViewingPlayerId(null)}
      />
    </Card>
  );
};
