...
export const ImageUploader: React.FC<ImageUploaderProps> = ({ label, maxFiles = 1, files, onChange, disabled, existingImages = [], onRemoveExisting, onErrorChange, requiredError }) => {
      ...
      {totalCount < maxFiles && (
        <div 
          ...
          className={`border-2 border-dashed rounded-lg p-6 flex flex-col items-center justify-center cursor-pointer transition-colors ${
            requiredError ? 'border-danger bg-danger/5 hover:bg-danger/10' : 'border-border hover:bg-border/30'
          } ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}
        >
          <UploadCloud className={`w-8 h-8 mb-2 ${requiredError ? 'text-danger' : 'text-muted'}`} />
          <p className={`text-sm ${requiredError ? 'text-danger font-medium' : 'text-muted'}`}>
            Click to upload {maxFiles > 1 ? 'images' : 'an image'} (Max {maxFiles})
          </p>
          <p className={`text-xs mt-1 ${requiredError ? 'text-danger/80' : 'text-muted/70'}`}>
            PNG, JPG up to 2MB
          </p>
        </div>
      )}

      {requiredError && totalCount === 0 && (
        <p className="text-xs text-danger font-medium mt-1">Avatar image is required to create a player.</p>
      )}
