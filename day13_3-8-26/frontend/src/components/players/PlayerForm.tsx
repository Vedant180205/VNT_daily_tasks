...
export const PlayerForm: React.FC<PlayerFormProps> = ({ 
  ...
  const [avatarRequiredError, setAvatarRequiredError] = useState(false);
  ...
  const onFormSubmit = (data: PlayerFormData) => {
    // Validate required avatar for new players
    if (!initialData && avatarFiles.length === 0) {
      setAvatarRequiredError(true);
      return;
    }
    setAvatarRequiredError(false);
