...
export const fetchPlayer = async (id: number): Promise<any> => {
...
export const autocompletePlayer = async (q: string): Promise<AutocompleteResult[]> => {
  const { data } = await apiClient.get('/api/players/autocomplete', { params: { q } });
  return data.data;
};
