...
export const TeamsPage: React.FC = () => {
  const { teams, loading, error, errorObject } = useTeams();
    ...
    <PageContainer>
      ...
      {loading ? (
        ...
        <ErrorState 
          ...
          isAccessDenied={errorObject && (errorObject as any).response?.status === 403}
          message={errorObject && (errorObject as any).response?.status === 403 ? (errorObject as any).response?.data?.message : undefined}
        />
      ) : teams.length === 0 ? (
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-border bg-gray-50/50">
                    <th className="px-6 py-4 text-[12px] font-bold text-muted uppercase tracking-wider">Team Name</th>
                    <th className="px-6 py-4 text-[12px] font-bold text-muted uppercase tracking-wider text-right">Roster Size</th>
                    <th className="px-6 py-4 text-[12px] font-bold text-muted uppercase tracking-wider text-right">Created At</th>
