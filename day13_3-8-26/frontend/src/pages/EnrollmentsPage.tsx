...
export const EnrollmentsPage: React.FC = () => {
  ...
  const { enrollments, pagination, loading, isFetching, error, errorObject, refetch, exportCSV } = useEnrollments({
    page,
    limit,
  ...
  return (
    <PageContainer>
      <DashboardHeader 
        totalPlayers={pagination?.total || 0} 
        title="Enrollments Management" 
        onExport={() => exportCSV({ page, limit, search: debouncedSearch, status, invite_type, role, team_id })}
        showExport={enrollments.length > 0}
      />
