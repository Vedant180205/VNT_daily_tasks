...
const authMiddleware = async (req, res, next) => {
    ...
    try {
        ...
        // 6. Validate the session in Redis
        const isValidSession = await sessionService.validateSession(decoded.id, token);
        if (!isValidSession) {
            const error = new Error("Session expired or logged out");
            error.statusCode = 401;
            return next(error);
        }

        // 7. Attach the decoded payload to the request object
        req.user = {
            id: decoded.id,
            email: decoded.email
        };
