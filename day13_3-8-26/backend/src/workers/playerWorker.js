...
const playerWorker = new Worker('players', async (job) => {
  ...
  try {
    ...
    if (uploadId) {
      await jobTrackerService.incrementJobProgress('upload', uploadId, 'completed', 1);
    }
    ...
    if (error.status) {
      if (uploadId) {
        await jobTrackerService.incrementJobProgress('upload', uploadId, 'failed', 1);
        await jobTrackerService.addJobError('upload', uploadId, {
          name: name || 'Unknown',
          email: email || 'Unknown',
          reason: error.message || 'Unknown error'
        });
