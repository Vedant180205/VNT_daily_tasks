...
class MvpRankingService {
  ...
  async syncLeaderboard(isMft = 1) {
    ...
    try {
      ...
      // 0. Start Sync Log
      syncLogId = await mvpModel.insertSyncLog();
      ...
      if (playersToUpsert.length === 0) {
        ...
        if (syncLogId) {
          await mvpModel.updateSyncLog(syncLogId, {
            status: 'SUCCESS',
            total_players: 0,
            logs_processed: 0,
      ...
      // 5. Redis Regeneration (Atomic Overwrite)
      try {
        console.log(`[MVP Sync] Refreshing Redis cache keys atomically...`);
        ...
        // Atomic cache warming (Overwrite without DEL to prevent misses)
      ...
      if (syncLogId) {
        await mvpModel.updateSyncLog(syncLogId, {
          status: 'SUCCESS',
          total_players: playersToUpsert.length,
          logs_processed: rawLogs.length,
      ...
      // Do NOT throw error to prevent cron server from crashing
  ...
  async getLeaderboard() {
    try {
      ...
      if (cached) {
        console.log(`[MVP API] Leaderboard served directly from Redis cache.`);
