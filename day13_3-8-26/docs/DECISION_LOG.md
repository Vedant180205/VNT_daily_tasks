...
## Decision: MVP Leaderboard Calculation & Redis Caching

**Stage:** MVP Ranking System Implementation

**Problem:**
Ranking 10,000+ players based on multiple match performances is computationally expensive. Calculating leaderboard totals dynamically on every API request causes unacceptable latency.

**Options:**
- Option A: Calculate dynamically using SQL aggregation on every request.
- Option B: Maintain a static `total_points` in the `users`/`players` table updated manually.
- Option C: Calculate via a scheduled sync script that aggregates logs (`mvp_performance_logs`), updates a final leaderboard table (`mvp_players`), and caches the top ranking in Redis.

**Chosen:** Option C – Scheduled Sync & Redis Cache.

**Reason:**
A separate sync script reduces DB load during peak API requests. Caching the finalized leaderboard in Redis (e.g., `mvp:leaderboard` with a 60-120s TTL) guarantees sub-millisecond read times for the frontend. The `mvp_players` table acts as a reliable persistence layer if Redis clears.

**Affected Files:**
- `backend/src/services/mvpRankingService.js`
- `backend/src/routes/mvpRoutes.js`

---

## Decision: Automated Background Sync vs Manual Trigger

**Stage:** Production-Level Leaderboard Sync

**Problem:**
The MVP Sync script required manual execution. In production, leaderboards need to automatically update without human intervention.

**Options:**
- Option A: Manual script execution via command line.
- Option B: Trigger sync via a webhook or admin API endpoint.
- Option C: Use `node-cron` to automatically schedule the script to run periodically (e.g., every 5 minutes).

**Chosen:** Option C – `node-cron` scheduled background job.

**Reason:**
Using `node-cron` fully automates leaderboard updates, matching real-world production setups. A sync log table (`mvp_sync_logs`) is introduced to track execution times and capture failures, ensuring visibility. The server handles failures gracefully without crashing.

**Affected Files:**
- `backend/src/jobs/cronScheduler.js`
- `backend/migrations/[timestamp]_create_mvp_sync_logs.sql`

---

## Decision: BullMQ Player CSV Import Architecture

**Stage:** Background Job Processing

**Problem:**
Importing a large number of players via CSV in real-time can block the main Node.js event loop and lead to timeouts, degrading the user experience.

**Options:**
- Option A: Process the CSV synchronously during the HTTP request.
- Option B: Use BullMQ and Redis to offload CSV processing to a background worker.

**Chosen:** Option B – BullMQ and Redis.

**Reason:**
A message queue guarantees processing and allows for independent scaling of workers. It also implements an automatic retry mechanism for transient failures.

**Affected Files:**
- `backend/src/workers/playerWorker.js`
- `backend/src/queues/playerQueue.js`
- `backend/src/queues/queueEvents.js`

---

## Decision: Notifications System

**Stage:** Admin & User Communication

**Problem:**
Need a way for admins to send announcements or updates to all users or specific roles.

**Options:**
- Option A: Build a full Service and Model layer for notifications.
- Option B: Bypass the service layer and have the Controller interact directly with the Model.

**Chosen:** Option B – Bypass the service layer.

**Reason:**
The notifications logic is simple (CRUD without complex business rules). Adding a service layer would create unnecessary boilerplate.
...
## Decision: Dashboard Master Payload & Redis Cache-Aside

**Stage:** Dashboard Data Aggregation

**Problem:**
The admin dashboard requires data from multiple tables (players, teams, enrollments, organizers, etc.). Fetching this sequentially or directly on every page load would put severe strain on the database.

**Options:**
- Option A: Fetch each dashboard widget data via separate API calls.
- Option B: Create a single "master payload" API that fetches data in parallel and caches the entire response in Redis.

**Chosen:** Option B – Master payload with Redis cache.

**Reason:**
Using `Promise.all` optimizes the initial DB fetch, and caching the aggregated JSON in Redis with a 300-second TTL prevents DB overload during traffic spikes.
...
## Decision: Atomic Redis Overwrite (No-DEL Pattern)

**Stage:** Caching Strategy Optimization

**Problem:**
The previous pattern for updating Redis caches was to `DEL` the key and then `SET` the new data. This created a brief window where a concurrent read could result in a cache miss.

**Options:**
- Option A: Continue with the `DEL` -> `SET` pattern.
- Option B: Use an atomic `SET` (with TTL overwrite) to replace the data seamlessly without a deletion gap.

**Chosen:** Option B – Atomic `SET` overwrite.

**Reason:**
Atomic overwrites eliminate the cache-miss window, ensuring that high-concurrency requests always hit the cache.
