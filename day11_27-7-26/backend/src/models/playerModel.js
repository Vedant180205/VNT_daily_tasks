...
const createPlayer = async (player) => {
    const { name, email, phone, team_id, avatar, gallery } = player;
    const [result] = await pool.query(
        "INSERT INTO players (name, email, phone, team_id, avatar, gallery) VALUES (?, ?, ?, ?, ?, ?)",
        [name, email, phone, team_id || null, avatar || null, gallery ? JSON.stringify(gallery) : null]
    );
...
const getAllPlayers = async ({ search, sort, order, limit, offset, team, date, status }) => {
    let whereClause = "WHERE 1=1";
    const whereParams = [];

    if (status === 'inactive') {
        whereClause += " AND players.is_deleted = TRUE";
    } else if (status === 'active') {
        whereClause += " AND players.is_deleted = FALSE";
    } else {
        whereClause += " AND players.is_deleted = FALSE"; // default
    }
...
const updatePlayer = async (id, player) => {
    const { name, email, phone, team_id, avatar, gallery } = player;
    const [result] = await pool.query(
        "UPDATE players SET name = ?, email = ?, phone = ?, team_id = ?, avatar = ?, gallery = ? WHERE id = ? AND is_deleted = FALSE",
        [name, email, phone, team_id || null, avatar || null, gallery ? JSON.stringify(gallery) : null, id]
    );
