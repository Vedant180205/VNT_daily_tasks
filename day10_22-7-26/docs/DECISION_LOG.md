...
**Stage:** Organizer Onboarding Redesign (Planned)

**Problem:**
The current `approveOrganizer` flow collapses the entire onboarding into one admin action: approve → immediately create `users` row with the organizer's pre-supplied password. This means:
- The organizer submits their password at application time (before approval), which is a UX and security anti-pattern.
- There is no email communication — the organizer has no way to know they have been approved.
- There is no formal rejection path (backend stub only).
- There is no document verification step as a distinct stage.
- Organizer cannot distinguish between "still pending", "approved but not registered", "documents under review", and "fully active".

**Options:**
- **Option A:** Keep the current single-step approval. Add email notification and rejection endpoint as patches.
...
- Pre-fill validation (`GET /api/organizer-registration/validate?token=...`): Hashes incoming token, verifies match, not expired, not used (`used_at IS NULL`), status = `REGISTRATION_PENDING` (2). Returns Phase 1 lead data to pre-fill form.
- Submission validation (`POST /api/organizer-registration/submit`): Re-hashes token, re-validates, creates `users` row, updates `organizers` profile data, marks `token_hash` used (`used_at = NOW()`), sets status to `REGISTRATION_COMPLETED` (3).

**Trade-offs:**
- Requires email service dependency (nodemailer).
...
- `backend/src/controllers/authController.js` – Phase 1 application submission, Phase 2 validate & complete handlers
- `backend/src/routes/adminRoutes.js` – admin endpoints
- `backend/src/routes/authRoutes.js` – public auth & registration endpoints
