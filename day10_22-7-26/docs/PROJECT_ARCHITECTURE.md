...
The system currently includes: authentication, JWT-based authorization, player/team CRUD, file uploads (avatar, gallery, documents), background CSV bulk import (BullMQ queue), Role-Based Access Control (RBAC) with an Organizer approval workflow, location data (countries/states/cities), and an Enrollments DataTable with server-side pagination and multi-filter support.

> **Planned:** The Organizer Onboarding workflow is being redesigned. The new flow introduces a multi-stage pipeline (Application → Pending Review → Approve/Reject → Registration Link → Documents Verified → Activated) with a one-time invitation token, a dedicated email service, and a separate document-verification step. See `docs/tasks/organizer-onboarding-redesign.md` for the full specification.
...
5. **Middleware (`/src/middleware`)**: Intercepts requests before controllers:
   ...
   - *(Planned)* `validateInvitationToken` – Verifies one-time registration token (signed, expiry-checked, single-use)
...
|-------|-----------|---------|
| `0` | `PENDING_REVIEW` | Initial application lead awaiting admin review |
| `1` | `REJECTED` | Lead application rejected by admin |
...
| `4` | `DOCUMENTS_UNDER_REVIEW` | Admin is reviewing uploaded KYC documents |
| `5` | `DOCUMENTS_REJECTED` | Documents rejected; requires correction/resubmission |
| `6` | `ACTIVE` | Organizer account fully activated (`is_active = 1`) |

The new status flow is strictly unidirectional and each transition is driven by an explicit admin action or organizer action, not implicit mutation.
...
### Organizer Signup (Current)
1. Organizer submits form to `POST /api/auth/signup-organizer` (multipart/form-data with documents).
2. `organizerService` saves record with `approval_status=0`, `is_active=0`.
3. Documents stored in `/uploads/organizers/documents/`.
4. On admin approval, a `users` row is atomically inserted and `organizers.user_id` is linked.
5. Organizer cannot login until admin approves.

### Organizer Signup (Planned – Onboarding Redesign)
The new flow decouples application submission from credential creation:
1. **Application Step**: Organizer submits application form (name, org, phone, Aadhaar, PAN, zone). No password is collected yet. Record saved as `approval_status=0`.
2. **Admin Review**: Admin views application, reads documents, and either Approves or Rejects.
   - **Reject**: Status set to `2`. Rejection reason stored. Optional notification email sent.
   - **Approve**: Status set to `1`. A cryptographically signed, one-time invitation token is generated, stored in a new `organizer_invitations` table with an expiry timestamp (e.g., 72 hours), and emailed to the applicant.
3. **Registration Step**: Organizer clicks the link (`GET /register?token=<token>`). Token is validated (exists, not expired, not used). Organizer sets their password and completes registration. Token is marked as used. A `users` row is created. Status advances to `3`.
4. **Document Verification**: Admin reviews the submitted documents and marks them verified. Status advances to `4`.
5. **Activation**: Admin activates the organizer. `is_active=1` is set. Status advances to `5`. Organizer can now log in and access all features.

### Admin Approval Flow (Current)
1. Admin calls `GET /api/admin/organizers/pending` to see pending list.
2. Admin calls `PATCH /api/admin/organizers/:id/approve`.
3. Approved: `approval_status=1`, `is_active=1`. A `users` row is created inside a DB transaction.

### Admin Approval Flow (Planned – Onboarding Redesign)
1. `GET /api/admin/organizers/pending` – list applications with `approval_status=0`.
2. `PATCH /api/admin/organizers/:id/approve` – generate token, email organizer, set status `1`.
3. `PATCH /api/admin/organizers/:id/reject` – store reason, set status `2`, optionally email.
4. `GET /api/admin/organizers/:id/verify-documents` – mark KYC complete, set status `4`.
5. `PATCH /api/admin/organizers/:id/activate` – set `is_active=1`, status `5`.
6. `POST /api/admin/organizers/:id/resend-invite` – regenerate and resend invitation token.
...
[Backend (Express)]
  ...
  ├── jsonwebtoken (Auth + Role Payload + Invitation Tokens) ← updated
  ├── bcrypt (Password Hashing)
  ├── multer (File Upload Handling)
  ├── nodemailer or SendGrid (Email Service – Planned)
...
- **Approval Flow Gap**: Current `approveOrganizer` immediately creates a `users` row, skipping a dedicated registration step — organizer never sets their own password.
- **No Email Notifications**: Organizers receive no communication on approval or rejection.
- **No Rejection Endpoint**: Frontend shows a Reject button but `handleReject()` is a stub — no backend route exists.
- **No Document Verification Step**: Documents are uploaded but never formally verified as a distinct workflow stage.

### Future Refactoring Opportunities
1. **Organizer Onboarding Redesign**: Implement the full multi-stage pipeline with invitation tokens and email service. See `docs/tasks/organizer-onboarding-redesign.md`.
2. **Schema Validation**: Adopt `Joi` or `Zod` for request body validation.
3. **Query Builder**: Use `Knex.js` to manage dynamic WHERE clauses cleanly.
4. **Refresh Tokens**: Add JWT refresh token rotation.
5. **TypeScript (Backend)**: Migrate backend to TypeScript for interface enforcement.
